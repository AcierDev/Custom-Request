...
try {
  ({ SharedBrandPill } = await import("./[id]/SharedBrandPill.tsx"));
} catch (error) {
  componentLoadError = error;
}

test("shared viewer brand pill renders text without the yellow E icon", () => {
  assert.ifError(componentLoadError);
  assert.equal(typeof SharedBrandPill, "function");

  const markup = renderToStaticMarkup(React.createElement(SharedBrandPill));

  assert.match(markup, />Everwood</);
  assert.match(markup, />Shared with you</);
  assert.doesNotMatch(markup, />\s*E\s*</);
  assert.doesNotMatch(markup, /amber/);
});
